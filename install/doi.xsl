...
<resource xmlns="http://datacite.org/schema/kernel-4" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://datacite.org/schema/kernel-4 http://schema.datacite.org/meta/kernel-4.2/metadata.xsd">
  ...
  <xsl:if test="not(//SUBELEMENT)">
    ...
    <dates>
      <date dateType="Created"><xsl:value-of select="//PUBLICATIONYEAR"/></date>
    </dates>
    ...
    <contributors>
        <contributor contributorType="HostingInstitution">
            <contributorName>CHANGEME</contributorName>
        </contributor>
    </contributors>
    <!-- // HOSTING INSTITUTION-->
  ...
  <xsl:if test="//SUBELEMENT">
    ...
    <titles>
        <title><xsl:value-of select="//METADATA-TitleDocMain"/></title>
    </titles>
    ...
    <xsl:if test="//NUMBER != ''">
      <descriptions>
        <description descriptionType="SeriesInformation"><xsl:value-of select="//ANCHORTITLE"/></description>
        <description descriptionType="SeriesInformation"><xsl:value-of select="//NUMBER"/></description>
      </descriptions>
    </xsl:if>
    <!-- // ANCHOR INFORMATION IF AVAILABLE -->
    ...
    <relatedItems>
      <relatedItem relationType="IsPublishedIn" relatedItemType="Journal">
        ...
        <publicationYear><xsl:value-of select="//PUBLICATIONYEAR"/></publicationYear>
        <volume><xsl:value-of select="//NUMBER"/></volume>
        <firstPage><xsl:value-of select="//SUBELEMENT-PAGE-START"/></firstPage>
        <lastPage><xsl:value-of select="//SUBELEMENT-PAGE-END"/></lastPage>
      </relatedItem>
    </relatedItems>
    <!-- // VOLUME INFORMATION WHERE THE SUB ELEMENT IS PART OF -->
...
- Specific metadata (e.g. TitleDocMain)
<xsl:value-of select="//METADATA-TitleDocMain"/>

- Specific person (e.g. Author)
<xsl:value-of select="//PERSON-Author"/>

- Type of Structure element (e.g. Article) if it is one
<xsl:value-of select="//SUBELEMENT"/>

- Start page of a structure element (e.g. [1])
<xsl:value-of select="//SUBELEMENT-PAGE-START"/>

- End page of a structure element (e.g. 23)
